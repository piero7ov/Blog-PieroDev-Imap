...
function isImagePart($part)
{
    ...
    if ($filename) {
        ...
        if (in_array($ext, ['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp'])) return true;
    }
...
function walkBodyParts($imap, $msgno, $parts, $prefix, &$out)
{
    foreach ($parts as $index => $part) {
        ...
        if (isset($part->type) && (int)$part->type === 0) {
            ...
            if ($subtype === 'HTML' && $out['html'] === null) $out['html'] = $raw;
...
// ----------------------------------------------------
// IMAGEN DESTACADA (primera encontrada)
// ----------------------------------------------------
...
function isAttachmentCandidate($part)
{
    ...
    if (isImagePart($part)) return false;           // NO imágenes aquí
...
// ----------------------------------------------------
// IMÁGENES (todas): listar + descargar
// ----------------------------------------------------
function walkImages($structure, $prefix, &$out)
{
    if (!isset($structure->parts)) return;
    ...
    foreach ($structure->parts as $index => $part) {
        ...
        if (isImagePart($part)) {
            ...
            // Si no hay filename, inventamos uno
            if (!$fn) {
                $ext = 'jpg';
...
function listImages($imap, $msgno)
{
    $structure = imap_fetchstructure($imap, $msgno);
    if (!$structure) return [];
...
if ($dlMsg > 0 && $dlImgIdx >= 0) {
    ...
    $bin = decodePart($raw, $img['encoding'] ?? 0);
...
<head>
    ...
    <style>
        :root{ --pad: 1.5rem; --gap: 1.25rem; --radius: 14px; }
        *{ box-sizing: border-box; }
        ...
        .site-header{
            ...
            gap: 1rem; padding: 1rem 1.1rem; margin-bottom: 1rem;
        ...
        .site-title{
            margin: 0; font-weight: 800; color: #2c3e50;
            font-size: clamp(1.1rem, 3.8vw, 1.6rem);
            line-height: 1.1; overflow-wrap: anywhere;
        }
        .site-subtitle{ margin: .35rem 0 0 0; color: #6b7280; font-size: .95rem; line-height: 1.25; }

        .social{ display:flex; align-items:center; gap: .55rem; flex-wrap: wrap; justify-content:flex-end; }
        .social a{
            width: 40px; height: 40px; display:grid; place-items:center;
            border-radius: 12px; background: rgba(255,255,255,.9);
            border: 1px solid rgba(44,62,80,.12); box-shadow: 0 8px 18px rgba(0,0,0,.06);
            transition: transform .15s ease, box-shadow .15s ease, background .15s ease;
        }
        .social a:hover{ transform: translateY(-1px); box-shadow: 0 12px 22px rgba(0,0,0,.10); background: #fff; }
        ...
        .social a:hover img{ filter: grayscale(0) opacity(1); transform: scale(1.06); }
        ...
        /* BUSCADOR */
        .searchbar{
            display:flex; gap: .6rem; align-items:center; margin: 0 0 1.25rem 0; flex-wrap: wrap;
        ...
        .post-cover{ position:relative; background:#e5e7eb; overflow:hidden; }
        .post-cover img{
            width:100%;
        ...
        .post-header{ padding: 1.1rem 1.35rem .75rem 1.35rem; }
        .post-title{
            margin:0;
        ...
        .badge-img{ border-color: rgba(85,107,47,.28); background: rgba(85,107,47,.10); color: #3f4f22; }
        .badge-att{ border-color: rgba(44,62,80,.18); background: rgba(44,62,80,.06); color: #2c3e50; }
        ...
        @media (max-width: 520px){
            ...
            .btn-read{ display:block; width:100%; text-align:center; }
        }
        ...
        .post-content iframe, .post-content frame{ display:none !important; }
        .post-content a{ color:#1a73e8 !important; text-decoration:none !important; }
        .post-content a:hover{ text-decoration:underline !important; }

        /* Ocultar SOLO imágenes cid: del HTML */
        .post-content img[src^="cid:"], .post-content img[src^="CID:"]{ display:none !important; }
        ...
        .att-item a:hover{ background:#fff; }
        .att-meta{
            font-weight:700;
            color:#6b7280;
            font-size:.85rem;
        ...
        .page-info{ color:#2c3e50; font-weight:800; }
    </style>
</head>
...
<div class="wrap">
    ...
    <?php else: ?>
        ...
        <form class="searchbar" method="get" action="<?php echo htmlspecialchars($self, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'); ?>">
            <input type="text" name="q" value="<?php echo htmlspecialchars($q, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'); ?>" placeholder="Buscar por asunto…">
        ...
        <?php if (!empty($emails)): ?>
            <div class="posts-grid">
                ...
                foreach ($frontEmails as $email_number):
                    ...
                    $images = listImages($inbox, $email_number);
                    $imgCount = count($images);
            ...
            <?php if ($totalPages > 1): ?>
                <div class="pagination">
                    ...
                    ?>
                    <a class="page-btn <?php echo ($page <= 1 ? 'disabled' : ''); ?>" href="<?php echo htmlspecialchars($prevUrl, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'); ?>">← Anterior</a>
                    <span class="page-info">Página <?php echo (int)$page; ?> de <?php echo (int)$totalPages; ?></span>
                    <a class="page-btn <?php echo ($page >= $totalPages ? 'disabled' : ''); ?>" href="<?php echo htmlspecialchars($nextUrl, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'); ?>">Siguiente →</a>
                </div>
            <?php endif; ?>
