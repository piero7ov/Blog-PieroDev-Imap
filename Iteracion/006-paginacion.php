...
<div class="wrap">
    ...
    <!-- ✅ HEADER CORPORATIVO + ICONOS (logos/...) -->
    ...
    <?php if ($viewMsg > 0): ?>
        <?php
            // ✅ volver conservando la página actual
            $backUrl = basename($_SERVER['PHP_SELF']);
            if ($page > 1) $backUrl .= '?page=' . $page;
        ?>
        ...
        <?php if (!empty($emails)): ?>
            <div class="posts-grid">
                ...
                foreach ($frontEmails as $email_number):
                    ...
                    // ✅ Leer más conserva la página
                    $link = basename($_SERVER['PHP_SELF']) . '?msg=' . (int)$email_number . '&page=' . (int)$page;
                ?>
                    <article class="post">
                        <?php if (!empty($post['image']) && !empty($post['image']['dataUri'])): ?>
                            <div class="post-cover">
            ...
            <?php if ($totalPages > 1): ?>
                <div class="pagination">
                    <?php
                        ...
                        $prevUrl = basename($_SERVER['PHP_SELF']) . '?page=' . $prev;
                        $nextUrl = basename($_SERVER['PHP_SELF']) . '?page=' . $next;
                    ?>
